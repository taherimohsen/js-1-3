...
*/
function getTransportModes(trMode) {
  let mode = trMode.slice(1);
  return mode;
}
...
*/
function isAccessibleByTransportMode(arrMode, strMode) {
  let isTrue = arrMode.some((el) => el === strMode);
  return isTrue;
}
...
function journeyPlanner(locations, transportMode) {
  ...
  let loc = locations.filter((el) =>
    isAccessibleByTransportMode(el, transportMode)
  );
  let trMode = loc.map((elt) => getLocationName(elt));
  return trMode;
}
