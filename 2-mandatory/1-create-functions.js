...
function tidyUpString(str) {
  let finalStr = str.map((toStr) => {
    return toStr.trim().replaceAll("/", "").toLowerCase();
  });
  return finalStr;
}
...
function remove(array, i) {
  let indexRemove = array.slice(0, i);
  let index = array.slice(i + 1, array.length);
  let indexRemover = indexRemove.concat(index);
  return indexRemover;
}
